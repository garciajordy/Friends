module UsersHelper
    ...
    def followbtn( user)
        ...
        if Following.where(user_id:current_user.id,follower_id: user.id ).any?
            (link_to "<i class='fa fa-2x fa-minus-square-o' aria-hidden='true'></i>".html_safe, following_path(@follow), method: :delete).html_safe
        else
        (link_to "<i class='fa fa-2x fa-plus-square-o' aria-hidden='true'></i>".html_safe, followings_path(user.id), method: :post).html_safe
        end
    end
